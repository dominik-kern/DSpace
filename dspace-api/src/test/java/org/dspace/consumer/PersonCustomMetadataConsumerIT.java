...
public class PersonCustomMetadataConsumerIT extends AbstractIntegrationTestWithDatabase {
    ...
    public void testAddCustomMetadata() throws SQLException {
        ...
        configurationService.setProperty("event.consumer.person-metadata.metadata", "local.nameHeader");
        configurationService.setProperty(
                "event.consumer.person-metadata.format", "{person.givenName}\\, {person.familyName}");

        callConsumerInitMethod();

        Item item = ItemBuilder.createItem(context, collection)
                .withTitle("TestPerson")
                .withMetadata("person", "givenName", null, "GivenName")
                .withMetadata("person", "familyName", null, "FamilyName")
                .build();
    ...
    public void testAddMultipleCustomMetadata() throws SQLException {
        ...
        configurationService.setProperty("event.consumer.person-metadata.metadata", "dc.title");
        configurationService.setProperty("event.consumer.person-metadata.format", "Test {person.givenName}");

        configurationService.addPropertyValue("event.consumer.person-metadata.metadata", "local.nameHeader");
    ...
    private void callConsumerInitMethod() {
        ...
        if (object instanceof Map) {
            ...
            ConsumerProfile consumerProfile = consumers.get("personcustommetadata");
            try {
